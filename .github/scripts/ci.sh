...
# The auKeep RodBusIntegrationTest is @Testcontainers(disabledWithoutDocker=true): it RUNS wherever
# Docker is present (i.e. CI), and starts an esquire-activemq:6.1.4 container. That image is local
# (built from activemq/Dockerfile), so it must exist before the reactor reaches auKeep's test phase
