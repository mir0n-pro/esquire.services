...
#
# THE COMPACT generator: single source of truth for the three datasources on the compact profile, emitted to
# BOTH compact deploy targets so they cannot drift:
#   * docker : compose-compact/o11y/grafana/provisioning/datasources/{loki,tempo,prometheus}.yaml
#   * k8s    : k8s-compact/charts/infra/grafana/templates/configmap-datasource.yaml   (helm template)
...
HEADER = ("# GENERATED by compose-compact/o11y/grafana/gen-datasources.py -- DO NOT EDIT BY HAND.\n"
...
def main():
    ...
    # THIS IS THE COMPACT GENERATOR: it writes ONLY into the compact trees, the same rule gen-dashboard.py
    # and gen-topology.py state. It was a byte-identical copy of the classic file and wrote into compose/ and
    # k8s/ -- so the compact datasource artifacts were never generated (hand-maintained under a DO NOT EDIT
    # header, the very drift this generator exists to remove) and running it mutated the classic tree.
    dsdir = os.path.join(svc_root, "compose-compact", "o11y", "grafana", "provisioning", "datasources")
    ...
    cm = [
        "{{- /* GENERATED by compose-compact/o11y/grafana/gen-datasources.py -- DO NOT EDIT BY HAND. */}}",
        "{{- /* Edit the generator and re-run it; it emits docker AND k8s so the two cannot drift. */}}",
        "apiVersion: v1",
        "kind: ConfigMap",
        "metadata:",
    ...
    p = os.path.join(svc_root, "k8s-compact", "charts", "infra", "grafana", "templates", "configmap-datasource.yaml")
    with open(p, "w", newline="\n") as f:
        f.write("\n".join(cm) + "\n")
    print("wrote %s" % p)
