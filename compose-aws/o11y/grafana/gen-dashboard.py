...
#
# THE CLASSIC generator: single source of truth for the "Esquire Services" board on the classic profile.
# Emits the SAME JSON to all THREE classic targets so they never drift:
#   * docker : compose/o11y/grafana/provisioning/dashboards/esquire-services.json
#   Writes ONLY compose-aws/o11y/grafana/provisioning/dashboards -- the classic generator owns
#   compose/, k8s/ and k8s-oci/, and neither writes the other's tree.
...
def build_panels():
    ...
    p.append(ts("Send-retry: backoff (avg ms)", 20, 112, 4, "ms",
                [tgt(avg_raw("messaging_retry_backoff_sum{%s}" % APP,
    ...
    # ---- No broker row ----
    # The classic board carries a Broker (ActiveMQ) row here: the bus seen from OUTSIDE, read from the
    # broker's own JMX agent. This lab has no broker of its own -- the transports are Amazon SNS, SQS and
    # Kinesis, and their server-side view lives in CloudWatch. So there is nothing to draw, and drawing it
    # anyway would put seven permanently empty panels on the board.

    # ---- Latency bands (4-layer request timing decomposed into timers, O1/T5-B) ----
    p.append(row("Latency bands", 120))
    ...
    p.append(ts("Request latency bands -- DERIVED (avg ms)", 8, 121, 8, "ms",
    ...
    p.append(ts("Request latency bands -- RAW (avg ms by layer)", 0, 121, 8, "ms",
    ...
    p.append(ts("Gateway total p95 by route (ms)", 16, 121, 8, "ms",
    ...
    p.append(row("Bandwidth", 129))
    p.append(ts("Service HTTP bytes OUT (B/s)", 0, 130, 8, "Bps",
                [tgt("sum by (application, instance) (rate(tomcat_global_sent_bytes_total{%s}[5m]))" % APP,
                     "{{application}} {{instance}}")],
                desc=BW_DESC))
    p.append(ts("Service HTTP bytes IN (B/s)", 8, 130, 8, "Bps",
    ...
    p.append(ts("Gateway EDGE bytes (B/s) -- client-facing", 16, 130, 8, "Bps",
    ...
    p.append(row("Business -- entity operations", 138))
    p.append(ts("Entity operations (ops/s by op + outcome)", 0, 139, 8, "ops",
    ...
    p.append(ts("Move queue -- depth (pending)", 8, 139, 4, "short",
                ...
                desc="The move backlog. /esq-move answers 202 at submit time and the work happens on the queue "
                     ...
                     "side would tell you."))
    p.append(ts("Move outcome (per s)", 12, 139, 4, "ops",
    ...
    p.append(ts("Dictionary lookups (by kind)", 16, 139, 8, "ops",
    ...
    p.append(row("Business -- money", 147))
    p.append(ts("Account transactions (tx/s by type + outcome)", 0, 148, 8, "ops",
    ...
    p.append(ts("Transaction latency (avg ms by type)", 8, 148, 8, "ms",
    ...
    p.append(ts("FX applied + accounts closed", 16, 148, 8, "ops",
    ...
    p.append(row("Business -- identity + token relay", 156))
    p.append(ts("Access profile reads + saves (by op + outcome)", 0, 165, 6, "ops",
    ...
    p.append(ts("Identity commands asked for (by command)", 6, 165, 6, "ops",
    ...
    p.append(ts("KeyCloak identity sync (by op + outcome)", 0, 157, 6, "ops",
    ...
    p.append(ts("KeyCloak sync latency (avg ms)", 6, 157, 6, "ms",
    ...
    p.append(ts("Token relay -- cache hit rate", 12, 157, 6, "percent",
    ...
    p.append(ts("Token relay -- KC /token acquire (avg ms)", 18, 157, 6, "ms",
    ...
    p.append(row("Business -- cache, keep + permissions", 173))
    p.append(ts("Tree cache -- broadcast dispatch (by outcome)", 0, 174, 8, "ops",
    ...
    ("aukeep" in ESQ_SERVICES) and p.append(ts("Audit keep -- DB writes (by op + outcome)", 8, 174, 8, "ops",
    ...
    p.append(ts("Permission checks (allow vs DENY)", 16, 174, 8, "ops",
    ...
    p.append(row("Resilience -- circuit breakers", 182))
    p.append(ts("Breaker state -- OPEN / half-open (1 = yes)", 0, 183, 8, "short",
    ...
    p.append(ts("Slow-call rate (%) -- the threshold that OPENS the breaker on a healthy backend", 8, 183, 8,
                "percent",
    ...
    p.append(ts("Failure rate (%) -- the threshold that opens on a backend that is actually failing", 16, 183, 8,
                "percent",
    ...
    p.append(ts("Calls through the breaker (by outcome)", 0, 191, 8, "ops",
    ...
    p.append(ts("Calls REFUSED by the breaker (the 503s the breaker itself caused)", 8, 191, 8, "ops",
    ...
    p.append(ts("Per-route deadline -- TimeLimiter outcomes", 16, 191, 8, "ops",
    ...
    p.append(row("Capacity -- cores in use (are we using the machine?)", 199))
    p.append(ts("Cores in use -- TOTAL across all services", 0, 200, 12, "short",
    ...
    p.append(ts("Cores in use by replica", 12, 200, 12, "short",
    ...
    p.append(ts("Effective CPUs the JVM sized itself for", 0, 208, 12, "short",
    ...
    p.append(ts("Host CPU -- the whole machine", 12, 208, 12, "percentunit",
...
def main():
    ...
    # THIS generator writes ONE tree: its own, compose-aws. It is a fork of the classic generator, and the
    # classic one writes compose/ + k8s/ + k8s-oci/ -- so a copy that kept those paths would silently
    # overwrite the classic, k8s and OKE boards with this lab's fork every time it ran. Neither generator
    # writes the other's tree.
    aws_dir = os.path.join(here, "provisioning", "dashboards")

    os.makedirs(aws_dir, exist_ok=True)
    for name, builder in (("esquire-services", build_dashboard),
                          ("esquire-logging", build_logging_dashboard)):
        path = os.path.join(aws_dir, "%s.json" % name)
