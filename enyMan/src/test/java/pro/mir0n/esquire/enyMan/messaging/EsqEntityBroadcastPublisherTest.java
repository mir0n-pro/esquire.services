...
class EsqEntityBroadcastPublisherTest {
    ...
    void publish_sendsToCorrectTopic() {
        publisher.publish(34, "100", EsqMsgConstants.EVENT_UPDATE, "rid1", "cid1", Map.of());
    ...
    void publish_bodyContainsAllCanonicalFields() throws Exception {
        publisher.publish(34, "100", EsqMsgConstants.EVENT_UPDATE, "rid1", "cid1", Map.of("state", "ACTIVE"));
    ...
    void publish_fixedValuesInBody() throws Exception {
        ...
        assertThat(parsed.get(EsqMsgConstants.FIELD_SERVICE_ID)).isEqualTo("enyMan");
    ...
    void publish_requestIdPropagated() throws Exception {
        publisher.publish(34, "100", EsqMsgConstants.EVENT_UPDATE, "my-request-id", "cid1", Map.of());
        String body = captureBodyJson();
        Map<String, Object> parsed = new ObjectMapper().readValue(body, Map.class);
    ...
    void publish_correlationIdPropagated() throws Exception {
        publisher.publish(34, "100", EsqMsgConstants.EVENT_UPDATE, "rid1", "my-correlation-id", Map.of());
        String body = captureBodyJson();
        Map<String, Object> parsed = new ObjectMapper().readValue(body, Map.class);
    ...
    void publish_requestIdGeneratedWhenNull() throws Exception {
        publisher.publish(34, "100", EsqMsgConstants.EVENT_UPDATE, null, null, Map.of());
        String body = captureBodyJson();
        Map<String, Object> parsed = new ObjectMapper().readValue(body, Map.class);
    ...
    void publish_jmsPropertiesMatchBodyFields() throws Exception {
        publisher.publish(42, "entity-99", EsqMsgConstants.EVENT_CREATE, "rid-x", "cid-x", Map.of());
        ...
        // JMS properties must carry the same values
        ...
        verify(mockMsg).setStringProperty(EsqMsgConstants.FIELD_SERVICE_ID,       "enyMan");
    ...
    void publish_textIsBodyOnlyNotJmsProperty() throws Exception {
        ...
        // Text must appear in body JSON (verified in other test)
        // Text must NOT be set as a JMS property
    ...
    void publish_applMsgIdIsUnique() throws Exception {
        ...
        // Execute both message creators to get bodies
