...
public class UpdateEntityHandler implements IBizTreeEventHandler {
    ...
    public void handle(String entityId, int entityKind, JsonNode textNode) throws Exception {
        boolean hasName    = textNode.has(EsqConstants.TEXT_NAME)    && !textNode.get(EsqConstants.TEXT_NAME).isNull();
        boolean hasDesc    = textNode.has(EsqConstants.TEXT_DESC);  // can be null
        boolean hasStatus  = textNode.has(EsqConstants.TEXT_STATUS)  && !textNode.get(EsqConstants.TEXT_STATUS).isNull();   // acc_status (pacMan/ACCT)
        boolean hasDeleted = textNode.has(EsqConstants.TEXT_DELETED) && !textNode.get(EsqConstants.TEXT_DELETED).isNull();  // usr_deleted_flg (enyMan/USR)
        if (hasName || hasDesc || hasStatus || hasDeleted) {
            long    pk         = Long.parseLong(entityId);
            String  name       = hasName    ? textNode.get(EsqConstants.TEXT_NAME).asText() : null;
            String  desc       = hasDesc    ? (textNode.get(EsqConstants.TEXT_DESC).isNull() ? null : textNode.get(EsqConstants.TEXT_DESC).asText()) : IBizTreeCacheRepository.SKIP;
            Integer statusCode = null;
            if (hasStatus) {
                statusCode = BizTreeConstants.decodeStatus(textNode.get(EsqConstants.TEXT_STATUS).asText());
            } else if (hasDeleted) {
                statusCode = BizTreeConstants.decodeStatus(textNode.get(EsqConstants.TEXT_DELETED).asText());
            }
