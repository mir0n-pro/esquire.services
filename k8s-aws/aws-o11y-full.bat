@echo off
rem ===========================================================================
rem aws-o11y-full.bat -- the SAME seven-piece stack, with every pillar pushed all the way.
rem
rem aws-o11y-on.bat arms observability AS SHIPPED. This one adds the two switches that
rem are deliberately off in the shipped default, because they are not free:
rem
rem   metricsHistograms=true   percentile buckets on every latency timer -- about 2.3x
rem                            the scrape (4,597 -> 10,686 series), paid on EVERY scrape
rem                            whether or not anyone looks. They also CARRY THE
rem                            EXEMPLARS: a trace id hangs off a bucket sample, so with
rem                            buckets off the metrics-to-trace hop is wired and inert.
rem   samplingRatio=1.0        every request traced, not a sample of them.
rem
rem Use it when you are investigating a percentile, or want the metric-to-trace hop to
rem work. It is not the arm to leave running.
rem
rem   logs     Loki + Alloy
...
echo.
echo === arming the services IN FULL (histograms on, sampling 1.0)
set ESQ_O11Y=full
...
echo.
echo === observability is ON IN FULL ^(histograms + sampling 1.0^). To look at it:
