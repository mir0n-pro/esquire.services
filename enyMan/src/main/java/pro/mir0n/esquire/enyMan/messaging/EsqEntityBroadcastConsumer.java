...
public class EsqEntityBroadcastConsumer {
    ...
    private void validateRequiredProperties(TextMessage message) throws JMSException {
        ...
        if (!EsqMsgConstants.BUS_ID_ENTITY.equals(busId)) {
            log.warn("EsqEntityBroadcastConsumer: unexpected BusID={}", busId);
        }
        if (!EsqMsgConstants.MSG_TYPE_ENTITY_BROADCASTS.equals(msgType)) {
            log.warn("EsqEntityBroadcastConsumer: unexpected MsgType={}", msgType);
        }
        if (!EsqMsgConstants.MESSAGE_ENCODING.equals(msgEncoding)) {
            log.warn("EsqEntityBroadcastConsumer: unexpected MessageEncoding={}", msgEncoding);
        }
        if (schemaVersion != EsqMsgConstants.SCHEMA_VERSION) {
            log.warn("EsqEntityBroadcastConsumer: unexpected SchemaVersion={}", schemaVersion);
        }
    }
