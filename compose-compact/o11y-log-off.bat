...
cd /d "%~dp0"
rem ===========================================================================
rem o11y-log-off.bat -- the BASELINE arm of the logging-cost isolation (I49) on the
rem COMPACT docker stack.
rem
...
rem
rem THIS, NOT o11y-off.bat, IS THE BASELINE ARM. o11y-off.bat leaves the application
rem logger at its default, so a cell taken against it still pays the ECS-JSON encode
rem per line, and the delta to the LOG arm comes out short.
rem
...
rem Mirror of compose\o11y-log-off.bat and k8s-compact\o11y-log-off.bat, over the four Java
...
rem           Windows filesystem, the slowest write on the box. Nothing ships those files, so they are
rem           not the log pillar; left on, the host disk I/O swamps the delta.
rem   MSG     same shape: its own file appender.
rem   AMQ/JMS org.apache.activemq + org.springframework.jms, INFO by default. No appender of their own,
rem           so they reach the root's ECS CONSOLE by additivity and DO emit. OFF in BOTH arms so the ON
rem           arm is "only pro.mir0n at INFO" and nothing else (mir0n).
...
echo.
echo LOGGING OFF (app logger off), tracing/metrics OFF (docker compact). The I49 baseline.
endlocal
