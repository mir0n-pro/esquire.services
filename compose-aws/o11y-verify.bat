...
rem
rem Run AFTER o11y-full.bat, and after some activity (e.g. an e2e run) so the
...
rem ===========================================================================
rem ONE FAIL IS EXPECTED HERE, and it is not a defect: the shared script asserts a scrape job named
rem `activemq`, which this lab does not run. The script is shared with the classic stack and is left exactly
rem as it is -- the checks that are specific to this lab live in aws-spec.bat instead, and that one must be
rem clean. Everything else on this run (the pillars, the chain, the datasource wiring, the boards) applies
rem here and is expected to pass.
set ENVNAME=docker
...
set BOARDS=compose-aws\o11y\grafana\provisioning\dashboards
