...
# The catalog bus-ids each AWS service carries, as a regex for the meter selector.
#
# The entity bus is carried by BOTH, and that is the shape of SNS rather than an ambiguity: SNS delivers and
# holds nothing, so a broadcast consumer owns an SQS QUEUE subscribed to the topic. The topic is the fan-out,
# the queues are where the messages wait. Counting the entity bus under both is therefore correct.
SNS_BUSES = "esquire[.]entity"
SQS_BUSES = "esquire[.]kc|esquire[.]entity"
KINESIS_BUSES = "audit-k"

# messaging_transport_up is tagged with the SERVICE's bus KEY (entity-bus / kc-bus / audit-bus), while the rod
# meters above are tagged with the CATALOG bus-id (esquire.entity / audit-k). Same label name, two vocabularies
# -- so the health selector cannot reuse the patterns above, and this is where that is written down.
SNS_LEGS = "entity-bus"
SQS_LEGS = "kc-bus|entity-bus"
KINESIS_LEGS = "audit-bus"
CARRIER_UP_FOR = 'min(messaging_transport_up{bus_id=~"%s"})' 

# ---- the carriers: beneath the lanes, one card per AWS service ----
#
# The classic board draws ONE broker here, and its numbers come from the broker's own JMX -- what is sitting in
# a destination, who is still consuming it, how full it is. There is no such view here, and pretending there is
# would be the worst kind of panel: SNS, SQS and Kinesis are managed, they run no JVM of ours, and their
# server-side counters live in CloudWatch, which this lab deliberately does not read.
#
# So every number on these three cards is the SERVICE's view of that transport -- the same meters the
# Messaging-bus row uses, filtered to the buses each carrier actually carries. That is honest about what can
# be known: it says whether traffic is moving and whether a leg is connected, and it does NOT claim to know a
# queue depth nobody here can see.
#
# WHICH LANE EACH ONE CARRIES is in BUSES below (`carrier`), not here, so the lines are drawn from the same
# fact that names them. Left to right they follow the lanes they serve -- Kinesis under the audit lane, SQS
# under the identity lane it owns outright, SNS on the right beside the entity lane it publishes into.
CARRIER_SEND = 'sum(rate(messaging_send_total{bus_id=~"%s"}[5m]))'
CARRIER_RECV = 'sum(rate(messaging_receive_total{bus_id=~"%s"}[5m]))'
CARRIER_HELD = 'sum(messaging_feed_depth{bus_id=~"%s"})'

comp("kinesis", "Amazon Kinesis",
     lambda r: health('(%s > bool -2)' % (CARRIER_UP_FOR % KINESIS_LEGS),
                      '(%s == bool 0)' % (CARRIER_UP_FOR % KINESIS_LEGS)),
     lambda r: [(CARRIER_SEND % KINESIS_BUSES, "ops", "put/s"),
                (CARRIER_RECV % KINESIS_BUSES, "ops", "read/s"),
                (CARRIER_HELD % KINESIS_BUSES, "short", "feed")],
     "/d/esq-services/", 193, 596)

comp("sqs", "Amazon SQS",
     lambda r: health('(%s > bool -2)' % (CARRIER_UP_FOR % SQS_LEGS),
                      '(%s == bool 0)' % (CARRIER_UP_FOR % SQS_LEGS)),
     lambda r: [(CARRIER_SEND % SQS_BUSES, "ops", "sent/s"),
                (CARRIER_RECV % SQS_BUSES, "ops", "recv/s"),
                (CARRIER_HELD % SQS_BUSES, "short", "feed")],
     "/d/esq-services/", 343, 596)

comp("sns", "Amazon SNS",
     lambda r: health('(%s > bool -2)' % (CARRIER_UP_FOR % SNS_LEGS),
                      # a leg that says DOWN is the only failure this side can see: the topic itself
                      # cannot be asked how it is
                      '(%s == bool 0)' % (CARRIER_UP_FOR % SNS_LEGS)),
     lambda r: [(CARRIER_SEND % SNS_BUSES, "ops", "pub/s"),
                (CARRIER_RECV % SNS_BUSES, "ops", "recv/s"),
                (CARRIER_HELD % SNS_BUSES, "short", "feed")],
     "/d/esq-services/", 493, 596)
...
LAYERS = [
    ...
    ["kinesis", "sqs", "sns", "collector"],                    # 2 --        the buses (lanes) + the carriers; the
...
ICON = {n: "img/icons/esq/%s.svg" % n for n in
        ("pacman", "postgres")}
# The three AWS marks, drawn in CorelDRAW like the rest of the set and copied in unchanged. They carry NO
# gradient on purpose: a gradient has to live in <defs>, and Grafana inlines every icon into one page and
# drops <defs> -- the square would then render with no fill at all. Solid colour on the shape instead.
# Bump ?v= whenever a drawing changes, or Grafana keeps serving the one it cached.
ICON["sns"] = "img/icons/esq/sns.svg?v=2"
ICON["sqs"] = "img/icons/esq/sqs.svg?v=2"
ICON["kinesis"] = "img/icons/esq/kinesis.svg?v=2"
...
def canvas():
    ...
    if BUSES_ON:
        # Each lane joins the carrier(s) that carry it, from below. The classic board has ONE broker under
        # every lane; here the lane says which service carries it, so a reader can see at a glance that the
        # identity bus is SQS and the broadcasts are SNS -- and that the audit bus has two options.
        for b in BUSES:
            for carrier in b.get("carrier") or []:
                if carrier not in C:
                    continue
                item = {"kind": "broker", "comp": carrier, "bus": b, "side": "T",
                        "ox": b["left"] + SPINE_W / 2.0, "oy": SPINE_TOP + SPINE_H}
                ends.setdefault((carrier, "T"), []).append(item)
                order.append(item)
    ...
    for item in order:
        if item["kind"] == "bus":
            ...
            b = item["bus"]
            by_name[item["comp"]]["connections"].append({
                "targetName": "bus:" + b["id"], "path": "straight",
                "source": item["anchor"],
                "target": {"x": 0, "y": -1},       # the bottom end of the lane
                "color": {"fixed": b["color"]}, "size": {"fixed": 2},
                # NO arrowhead: nothing flows "to" or "from" the carrier -- it IS the medium the flow happens in.
                "direction": "none",
            })
...
def build():
    ...
    # The classic board has two panels here read from the BROKER: queue depth, and consumers per destination.
    # Both are gone, because neither number exists in this lab. SNS, SQS and Kinesis are managed -- there is no
    # JVM of ours to scrape, and their own counters are in CloudWatch, which this lab does not read. Drawing
    # them anyway would put two permanently empty panels beside a healthy picture.
    #
    # What CAN be known from here is the receiving side of each leg: how deep its feed is, and whether it is
    # connected at all. That is strictly less than a broker's view -- it cannot see a destination whose
    # consumer has died, because a dead consumer reports nothing -- and the panel says so rather than pretending.
    p.append(ts("Feed depth per bus  (the receiving side, from the service)", 8, 26, 8, "short",
                [tgt("sum by (bus_id) (messaging_feed_depth)", "{{bus_id}}")],
                desc="What a consumer has taken off the transport and not yet applied. Flat at zero is healthy; "
                     "climbing means the workers are slower than the traffic. NOTE the limit: this is the "
                     "CONSUMER's own count, so a consumer that has died reports nothing at all rather than a "
                     "backlog -- pair it with the transport panel to the right, which goes to 0 when a leg drops."))
    p.append(ts("Legs connected per bus  (1 up, 0 down, -1 not yet known)", 16, 26, 8, "short",
                [tgt("min by (bus_id) (messaging_transport_up)", "{{bus_id}}")],
                desc="The worst leg on each bus. -1 is not a fault: a leg seeds UNKNOWN and only claims UP once a "
                     "message has actually moved, so a quiet bus reads -1 until it carries something. 0 is the "
                     "fault -- a send or a receive failed."))
...
def main():
    ...
    # THIS generator writes ONE tree: its own, compose-aws. It is a fork of the classic generator, which
    # writes compose/ + k8s/ + k8s-oci/ -- so a copy that kept those paths would overwrite the classic, k8s
    # and OKE topology boards with this lab's fork every time it ran. Neither writes the other's tree.
    for path, model, lefts, drop_comp, drop_bus, bus_left in (
            (os.path.join(here, "provisioning", "dashboards",
