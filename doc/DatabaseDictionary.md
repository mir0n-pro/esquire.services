...
|---|---|---|
| `ESQ_REF_SEQ` | 1 | ESQ_ADDRESS — profile reference PKs |

`ESQ_ACCT_TRANSACTION.ATR_PK` is a `VARCHAR(64)` UUID generated by the application — no sequence.
**One sequence is all the schema has**, and the reason is the next section.

### Entity keys are minted by the application

**The key concept of this schema: an entity key is unique across ALL business entities.** An org, a user
and an account never share a key value — the numbers come from one space, not three. That is what makes
the shared-key satellite work: an entity key on its own is enough to find its row in ESQ_ENTITY_PATH,
without knowing which of the three tables the entity lives in.

One space of keys is exactly what a database sequence is normally for. **We do not use one.** The key is
minted by the application, from the clock:

```
entity id = (milliseconds since the Esquire era) * 10000
          + instance number                      * 1000
          + sequence within the millisecond      % 1000
```

The Esquire era is 26 June 2025, 13:20 EDT. The bottom four decimal digits are split one and three: one
digit for the instance, three for a counter within the millisecond. So the shape allows **up to 10
instances, each minting up to 1000 keys per millisecond**, and an instance number outside 0–9 fails at the
first mint rather than quietly overflowing into the time digits.

It stays a single `BIGINT`, so the globally-unique numeric key invariant holds and the path satellite still
joins on a plain number.

**Only enyMan mints them.** Every create routes through that one service, so key generation lives in
exactly one place — which is also why no coordination is needed: within one instance the counter is local,
and between instances the instance digit keeps the spaces apart.

---

## The change number

Every entity and sub-entity row carries a `*_CHANGE_NO`: a count that starts at 1 and goes up by one each
time that row is written. It says **which version of the row you are looking at**.

It exists because two things needed an order and had none:

- **The audit trail.** Records were sorted by a random key and a timestamp, so two changes to the same row
  inside the same clock tick had no reliable order between them.
- **The receivers.** An entity change is announced on a message. Messages can arrive twice, or out of
  order. Without a number on the row there is no way to tell a repeat from a new change, or a late message
  from a current one.

### Why not a database sequence

**For one database serving one application instance, a sequence is the normal answer** and it works well.
It is the wrong answer here, and the reason is worth stating plainly, because the alternative looks simpler
until you scale it.

A global sequence has to be **in true order** and **fast**, and those two pull against each other:

- Let each instance take a block of numbers ahead of time and it is fast — but each instance is handing out
  its own block, so the numbers are not in true order across the cluster.
- Force strict order and every single number needs the instances to agree — a cheap local read becomes
  cross-instance coordination, which is exactly the thing that does not scale.
- Split the data across partitions or shards and there is no single place for the sequence to live at all.

This is not particular to one product: it is the same in every clustered database of this shape — Oracle
RAC and the shared clusters like it, and the multi-instance Postgres arrangements.

**Our own schema already pays that price, and it is the clearest example available.** The one sequence we
keep, `ESQ_REF_SEQ`, is declared `CACHE 1` on Postgres and `NOCACHE` on Oracle. That is the setting that
keeps the numbers in strict order — by refusing to hand out a block ahead of time — and it is precisely the
slow end of the trade above: every single number costs a trip to the sequence.

### Why a per-row counter avoids all of it

The change number is **not global**. It belongs to the row, and it only has to grow within that one row's
own history. Two different rows never compare their numbers, so no instance ever has to agree with another
about anything.

And it costs nothing to maintain. The write path **already holds a lock on the row it is about to write**:
the service reads the row `FOR UPDATE`, raises the number in hand, and writes it back as an ordinary value
in the same statement. No extra read, no extra round trip, no coordination.

The one exception is `ESQ_ENTITY_PATH`, where a move rewrites many rows under a single lock and there is no
per-row read to raise anything in — so there the number is raised inside the SQL. See
[ESQ_ENTITY_PATH](#esq_entity_path).

### The limit, stated honestly

This gives a **per-row order, not a system-wide order of events**. You can put one row's history in true
order, and you can tell a repeated or out-of-date message about that row from a current one. You cannot use
these numbers to say whether a change to one account happened before a change to a different user — they
count different histories, and comparing them means nothing.

If a global order across different entities is ever needed, that is a separate mechanism and this column is
not it.
...
Every entity table carries four audit columns written by the service layer on every mutation:

| Column suffix | Type | Description |
|---|---|---|
| `*_CHANGE_NO` | BIGINT NOT NULL | Change number — raised by one on every write, so the row's history has a true order |
...
The last three say **who and which request**; the change number says **which version**. See
[The change number](#the-change-number) for how it behaves and why it is not a database sequence.
...
`*_LOG` table carrying the same columns plus `*_ACTION VARCHAR(1)` (`I`/`U`/`D`), `*_ACTION_TS TIMESTAMP`
and the row's `*_CHANGE_NO`.
...
The keys come from one space without a database sequence — see
[Entity keys are minted by the application](#entity-keys-are-minted-by-the-application).
...
| ORG_CHANGE_NO | BIGINT NOT NULL | NUMBER(16,0) NOT NULL | Change number — raised by one on every update of this row |
...
| USR_CHANGE_NO | BIGINT NOT NULL | NUMBER(16,0) NOT NULL | Change number — raised by one on every update of this row |
...
| ACC_CHANGE_NO | BIGINT NOT NULL | NUMBER(16,0) NOT NULL | Change number — raised by one on every update of this row (a posted transaction raises it too, through the balance update) |
...
| EP_CHANGE_NO | BIGINT NOT NULL | NUMBER(16,0) NOT NULL | Change number of the PATH — its own counter, separate from the entity row's |
...
**Why the path has a counter of its own.** A move rewrites the whole subtree with one prefix update here,
but updates only the moved entity's own row in ESQ_ORG / ESQ_USER / ESQ_ACCOUNT. So a descendant's entity
number never moves, even though its path did — the path row is the only place that change can be counted.
The two numbers are never compared with each other: they count different things.

It is also the one change number **raised inside the SQL** (`ep_change_no = ep_change_no + 1`) rather than
read and raised by the service. Everywhere else the row is read under its own `FOR UPDATE` lock, so the
service holds the row and can raise the number in hand. A move takes ONE lock for the whole path table and
then rewrites many rows at once — there is no per-row read to raise anything in.
...
| PE_CHANGE_NO | BIGINT NOT NULL | NUMBER(16,0) NOT NULL | Change number — raised by one on every update of this row |
...
| AD_CHANGE_NO | BIGINT NOT NULL | NUMBER(16,0) NOT NULL | Change number — raised by one on every update of this row |
...
| UPR_CHANGE_NO | BIGINT NOT NULL | NUMBER(16,0) NOT NULL | Change number — raised by one on every update of this parameter row |
...
| OPR_CHANGE_NO | BIGINT NOT NULL | NUMBER(16,0) NOT NULL | Change number — raised by one on every update of this parameter row |
...
| AU_CHANGE_NO | BIGINT NOT NULL | NUMBER(16,0) NOT NULL | Change number — raised by one on every update of this row |
...
| TREE_ENTITY_CHANGE_NO | BIGINT | The entity change number last applied to this node |
| TREE_PATH_CHANGE_NO | BIGINT | The path change number last applied to this node |

**The two change numbers are what let the cache ignore an event it has already seen.** The entity
broadcast is a topic: it fans out, and each service applies it on a pool of workers with no per-entity
affinity, so two events for the same node can be applied out of order, and a redelivered one can arrive
long after the change it carries was applied. Before applying, the cache compares the arriving number
against the one stored here and skips anything that is not newer; after applying, it stamps the new one.

**There are TWO because they count different things and must never be compared with each other.** A
create, update or delete carries the ENTITY row's number; a move (a path event) carries the PATH row's
number. A move rewrites every descendant's path while leaving their entity rows untouched, so a
descendant's path number advances while its entity number does not. One shared column would make each
kind of event look stale to the other — descendant path events would be skipped and a moved subtree would
be left half-repathed.

**Both are nullable, and the main reason is the virtual nodes.** A virtual folder node exists only in this
cache — it has no row in ESQ_ORG / ESQ_USER / ESQ_ACCOUNT and none in ESQ_ENTITY_PATH, which is why
`TREE_ENTITY_PK` is null for it as well. There is no row behind it for anything to count, so it has no
change number and never will. It is not a missing value; there is nothing to miss.

Two lesser cases are nullable for the same practical reason: a node loaded before the columns existed, and
an event from a producer that sends no number. All three apply unguarded, which is the safe direction — an
unguarded apply writes current data, while a wrong skip would leave the cache stale until the night-watch
heals it.
...
| ATR_ACC_CHANGE_NO | BIGINT | NUMBER(16,0) | The ACCOUNT change number this transaction produced — nullable |
...
**`ATR_ACC_CHANGE_NO` is the account's number, not the transaction's.** The transaction row has no counter
of its own and never will: it is written once and never updated, so there would be nothing to count. What
this column holds is the version the ACCOUNT reached because of this posting — `ATR_ACC_PK` says which
account, `ATR_ACC_CHANGE_NO` says which of its history records. That lets the ledger be reconciled against
the account history by number instead of by matching on time:

```
esq_acct_transaction.atr_acc_change_no  =  esq_account_log.accl_change_no
   (for the same account)
```

A transfer writes two rows, one per leg, and each carries its own account's number — nothing has to say
which leg it belongs to.

It is **nullable on purpose**: the column is informative rather than functional, and a row written before
it existed has no honest value to carry.
...
| `*_CHANGE_NO` | BIGINT NOT NULL | NUMBER(16,0) NOT NULL | The change number of the logged row — which version of that row this record is |

The change number is what puts an entity's history back into true order. Sorting by the action timestamp
cannot: two changes to the same row inside the same clock tick have no reliable order between them, and in
the decoupled models the timestamp records when the log row was *written*, not when the change happened.

**It is the one log column that is NOT NULL.** Every other column is nullable, so that a delete record with
an empty body still inserts. This one is required, because a log row that cannot say which version it
records is a row nothing can order — and a record that cannot be ordered is not much of an audit record.
A write that cannot supply it is a defect, and it fails loudly instead of writing an unusable row.
...
(one btree per log table: `ADL_/PEL_/USRL_/AUL_/ORGL_/ACCL_/UPRL_/OPRL_ACTION_TS`, each named
...
Mirrors ESQ_ORG. Prefix `ORGL_`.

---

### ESQ_ACCOUNT_LOG

Mirrors ESQ_ACCOUNT. Prefix `ACCL_`.
