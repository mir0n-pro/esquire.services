...
-   **The trace view is per-instance.** The trace store labels a span by the emitting replica; the logical service name is preserved on logs and metrics. When metrics arrive, correlating a trace to a metric by service name will need this difference reconciled.
...
Spans are exported over **OTLP** — a vendor-neutral wire — through an **OpenTelemetry Collector**, so the trace backend is swappable without touching a service. The Esquire stack stores traces in **Grafana Tempo** and views them in **Grafana**, the same pane as the logs; any OTLP-compatible backend — Tempo, Jaeger, Zipkin — works the same way.

-   **The waterfall**: the trace UI shows a request as a "Gantt chart" of spans across services and bus hops. Because logs carry the same id (traceId == Correlation ID), a span links straight to the log lines Esquire produced at that exact moment, and back.
