...
rem THE SHIPPED LEVELS ARE SET EXPLICITLY, not left to the chart default. This arm upgrades with
rem --reset-then-reuse-values, which KEEPS whatever a previous arm set -- so after o11y-full-on (levelMir0n
rem INFO, develop/msg/amq/jms OFF) this one would leave those in place and report "as shipped" while the
rem application was still logging at INFO. An arm must state every knob it owns.
  call helm upgrade esquire-%%s charts\esquire-%%s --reset-then-reuse-values --set observability.enabled=true --set observability.metricsHistograms=true --set logging.levelMir0n=DEBUG --set logging.levelDevelop=DEBUG --set logging.levelMsg=DEBUG --set logging.levelAmq=INFO --set logging.levelJms=INFO --force-conflicts
