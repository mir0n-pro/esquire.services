 ...
 * client_id, or a cached jti, is served a valid downstream JWT.
 *
 * Where that is harmless, precisely: the allowlists are EMPTY by default and empty on OKE, and the two targets
 * that DO arm the relay -- docker compact and local k8s, both for hauberk load runs -- publish the client
 * secret anyway, so knowing the client_id grants nothing the published secret does not already grant. The
 * targets where a credential is real are the targets where the relay is off. If the relay is ever taken to
 * production,
