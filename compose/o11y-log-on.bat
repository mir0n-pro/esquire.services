...
cd /d "%~dp0"
rem ===========================================================================
rem o11y-log-on.bat -- LOGGING ONLY: the log pillar on, tracing + metrics OFF.
rem
rem THE POINT (I49, mir0n): what does LOGGING cost, BY ITSELF?
rem T10's perf matrix measured o11y as ONE lump -- "o11y OFF" vs "o11y ON" -- so its
rem -11.5% / -24% is all three pillars together and no one can say which bought what.
rem (T10 is SOUND: the app logged at DEBUG throughout and its ON arm really did ship
rem those lines to Loki. Only the split between pillars was missing.)
rem
...
rem identically in both arms and cancelled out. The OFF arm (o11y-log-off.bat) now sets
...
rem
rem Mirror of k8s\o11y-log-on.bat.
rem ===========================================================================
...
rem Windows BIND MOUNT (26 GB) that nothing ships; AMQ/JMS reach the ECS console by additivity and would
rem add log volume that is not the thing being priced. See o11y-log-off.bat for the full reasoning.
...
echo.
echo LOGGING ON, tracing/metrics OFF (docker). Grafana: http://localhost:3009  (admin/admin)
